...
public:
    ZedSegmentation() : Node("zed_segmentation") {
        ...
        this->declare_parameter<int>("max_planes", 2);              // cari plane terbesar + satu tambahan
        this->declare_parameter<int>("min_plane_inliers", 500);     // minimal inlier agar plane valid
        this->declare_parameter<double>("plane_distance_thresh", 0.02); // ambang jarak titik-ke-bidang (meter)
...
private:
    ...
    void cloudCb(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
        ...
        // ---- Prepare colored cloud (default RED for non-planar) ----
        ...
        // iterate to find up to max_planes planes
        for (int plane_id = 0; plane_id < max_planes; ++plane_id) {
            ...
            if ((int)inliers.indices.size() < min_plane_inliers) {
                // Tidak cukup inlier untuk menyebut plane
                break;
            }

            // Warna inlier: plane terbesar pertama = GREEN, berikutnya = ORANGE
            uint8_t R = 0, G = 255, B = 0; // GREEN
            if (plane_id >= 1) { R = 255; G = 165; B = 0; } // ORANGE (RGB approx)
            ...
            // Buat remaining_indices baru dengan menghapus inliers
