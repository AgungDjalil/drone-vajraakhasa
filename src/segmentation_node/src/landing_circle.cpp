...
public:
  SafeLandingColor() : Node("safe_landing_color")
  {
    ...
    // Frame TF
    plane_frame_    = declare_parameter<std::string>("plane_frame",   "odom"); // tempat proyeksi & perhitungan
    ...
    RCLCPP_INFO(get_logger(),
      "SafeLandingColor: plane_frame=%s axes=%s safe_d=%.2fm grid=%.2fm fill>=%.2f pts/sel>=%d inflate=%d votes(window=%d tol=%.2fm min=%d) -> TF '%s' in '%s'",
...
private:
  // ===== util proyeksi u,v di plane_frame =====
        ...
        if (plane_frame_ != odom_frame_) {
          try {
            auto T_odom_from_plane = tf_buffer_->lookupTransform(
              odom_frame_, plane_frame_, msg->header.stamp, rclcpp::Duration::from_seconds(0.1));
  ...
  // ===== publish point dalam disk aman + centroid (di plane_frame) =====
                       ...
                       bool* out_have_centroid=nullptr)
  {
    ...
    if (have_center) {
      ...
      for (size_t i=0;i<cloud.size();++i) {
        ...
        if (du*du + dv*dv <= r2) {
          // tampilkan posisi asli (agar bentuk cloud tetap), atau pakai P_plane sesuai kebutuhan visual
          const auto& p = cloud[i];
          pcl::PointXYZRGB q;
    ...
    out.header = hdr; // frame_id tetap frame cloud; itu hanya untuk visualisasi titik hijau
    pub_->publish(out);
